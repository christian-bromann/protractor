  ...
  var ret = function(done) {
    ...
    function rejectFlow(err) {
      if(typeof global._MAX_PRTRCTR_RETRIES === 'number' && ++global._PRTRCTR_RETRIES < global._MAX_PRTRCTR_RETRIES) {
        ...
        return flow.execute(controlFlowExecute, runnable.fullTitle()).then(resolveFlow, rejectFlow);
      }

      return done(err);
    }

    flow.execute(controlFlowExecute, runnable.fullTitle()).then(resolveFlow, rejectFlow);
  };
